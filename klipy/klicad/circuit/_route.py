...
HANDOFF Phase F.  Opt-in: passing route=True to to_klicad_sch() makes
...
# (one KliCAD grid step) keeps wires visibly off the body.
KEEPOUT_MM = 2.54
...
def route_signal_nets(c: "Circuit", kicad, placed: dict[str, str]) -> int:
    ...
    # different nets (KliCAD merges collinear/coincident wires at save).
    ...
    # etc.  If a signal wire touches one, KliCAD connects the signal
    ...
    for net_name, terminals in nets.items():
        ...
        if not all_ok:
            ...
            # connectivity by KliCAD's "same-name" rule.
        ...
        # KliCAD's connection-graph requires an explicit junction at any
        # point where 3+ wire endpoints meet, or where a wire crosses
        # a pin mid-segment.  We add liberally; KliCAD drops junctions
...
def _hanan_grid(
    ...
    with a pin causes KliCAD's save-time wire merge to collapse two
...
def _route_one_net(
    ...
    endpoint, which is necessary for KliCAD's save-time wire-merge logic
    ...
    # KliCAD merges, hiding the pin behind a continuous wire and
