...
``KliCAD.run_python(code) -> RunPythonResult``.  Power users can call it
...
snippet, ships it to KliCAD's embedded interpreter via ``run_python``,
...
``KliCAD`` via :attr:`KliCAD.kicad` (or call :meth:`KliCAD.run_python`
directly).
"""
