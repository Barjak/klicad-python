...
Owns the underlying :class:`kipy.kicad.KiCad` connection (which it
either receives or constructs with a 300 s timeout against the default
``/tmp/kicad/api.sock``).  Each sub-proxy is wired with a shared
...
class KliCAD:
    ...
    To reuse an existing :class:`kipy.kicad.KiCad` connection, pass it as
    ``kicad=...``::
    ...
    def __init__(
        self,
        kicad: Optional["KiCad"] = None,
        *,
        socket_path: Optional[str] = None,
        timeout_ms: Optional[int] = None,
    ) -> None:
        """Connect to a running KliCAD/KiCad instance.

        :param kicad: an existing :class:`kipy.kicad.KiCad` to reuse.  If
        ...
        """
        if kicad is None:
            # Imported lazily so importing kipy.klicad doesn't drag in
    ...
    def kicad(self) -> "KiCad":
        """The underlying :class:`kipy.kicad.KiCad` connection.
    ...
    def run_python(self, code: str):
        """Pass-through to :meth:`kipy.kicad.KiCad.run_python`.

        Use for arbitrary code that doesn't fit any sub-proxy.  Returns
        the raw :class:`kipy.kicad.RunPythonResult`.
        """
        return self._kicad.run_python(code)

    def close(self) -> None:
        """Close the underlying KiCad connection."""
        self._kicad.close()
