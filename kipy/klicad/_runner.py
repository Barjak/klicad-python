"""Internal helpers for invoking ``KiCad.run_python`` from façade proxies.

Each public proxy in :mod:`kipy.klicad` ultimately routes through
...
2. Calls ``KiCad.run_python(snippet)``.
...
if TYPE_CHECKING:  # pragma: no cover
    from kipy.kicad import KiCad, RunPythonResult
...
class KliCADError(RuntimeError):
    ...
    captured by KiCad's embedded interpreter (``exception_traceback`` from
    :class:`kipy.kicad.RunPythonResult`).  ``stdout`` and ``stderr`` carry
...
class _PyRunner:
    """Wraps a :class:`kipy.kicad.KiCad` instance to dispatch ``run_python``
